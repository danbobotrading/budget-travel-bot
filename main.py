...
MENU, TRIP_TYPE, FROM_CITY, TO_CITY, DEPART_DATE, RETURN_DATE, HOTEL_CITY, CHECKIN, CHECKOUT = range(9)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "✈️ Welcome to *Budget Travel Deals*\nChoose a service:",
...
async def menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Coming soon. Choose Flights, Buses or Hotels.")
    return MENU
...
async def from_city(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not city:
        await update.message.reply_text(f"City not found. Choose from: {', '.join(CITIES)}")
        return FROM_CITY
...
async def depart_date(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
    ...
    if context.user_data["trip_type"] == "Round-trip":
...
def main():
    ...
    conv = ConversationHandler(
        ...
        states={
            ...
            TRIP_TYPE: [MessageHandler(filters.TEXT, trip_type)],
            FROM_CITY: [MessageHandler(filters.TEXT, from_city)],
            TO_CITY: [MessageHandler(filters.TEXT, to_city)],
            DEPART_DATE: [MessageHandler(filters.TEXT, depart_date)],
